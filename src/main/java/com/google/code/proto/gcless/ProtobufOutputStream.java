...
final public class ProtobufOutputStream {
    ...
    public static int writeRepeatedUuid(final int fieldNumber, final List<UUID> value, byte[] buffer, int position) {
        if (value.isEmpty()) {
            return position;
        }
        int result = position;
        for (int i = 0; i < value.size(); i++) {
            result = writeUuid(fieldNumber, value.get(i), buffer, result);
        }
        return result;
    }
    ...
    public static int writeUuid(final int fieldNumber, final UUID value, byte[] buffer, int position) {
        ...
        int result = ProtobufOutputStream.writeTag(fieldNumber, WIRETYPE_VARINT, buffer, position);
        return ProtobufOutputStream.writeRawBytes(curMessageData, buffer, result);
    }
